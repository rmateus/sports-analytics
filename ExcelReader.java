	...
	public void read() throws IOException  {
		...
		try {
			...
			for (int i =2; i< sheet.getRows(); i++){
				teamArr[i-2] = new Team(sheet.getCell(j,i).getContents(), null, Integer.parseInt(sheet.getCell(j+1,i).getContents()), Integer.parseInt(sheet.getCell(j+2,i).getContents()));
				j=0;
			}
			/*
			int i=0;
